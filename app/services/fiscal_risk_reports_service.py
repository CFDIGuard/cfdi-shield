...
def _rr1_motives(invoice: Invoice, uuid_counts: Counter[str]) -> list[str]:
    motives: list[str] = []
...
def build_fiscal_risk_reports(invoices: list[Invoice]) -> dict[str, list[dict[str, object]]]:
    ...
    rr9_scores = calculate_rr9_score(grouped_by_rfc)

    rr9_rows: list[dict[str, object]] = []
    for rfc_emisor, provider_invoices in grouped_by_rfc.items():
        rr9_metrics = rr9_scores.get(rfc_emisor)
        if not rr9_metrics:
            continue
        provider_name = _safe_text(provider_invoices[0].razon_social)
        if not rr9_metrics["risk_reason"]:
            continue
        ...
        rr9_rows.append(
            {
                ...
                "facturas": rr9_metrics["facturas"],
                "total_mxn": rr9_metrics["total_mxn"],
                "canceladas": rr9_metrics["canceladas"],
                "porcentaje_canceladas": rr9_metrics["porcentaje_canceladas"],
                "score_riesgo": rr9_metrics["score_riesgo"],
                "monedas_usadas": rr9_metrics["monedas_usadas"],
                "operaciones_repetidas": rr9_metrics["operaciones_repetidas"],
                "riesgo_acumulado": str(rr9_metrics["risk_level"]).replace("LOW", "BAJO").replace("MEDIUM", "MEDIO").replace("HIGH", "ALTO"),
                "motivo": rr9_metrics["risk_reason"],
                "flag_requiere_contrato": rr9_metrics["flag_requiere_contrato"],
            }
        )

    rr9_rows.sort(key=lambda row: (-float(row["total_mxn"]), str(row["rfc_emisor"])))

    resumen_riesgos = [
        {"indicador": "CFDI con riesgo RR1", "valor": len(rr1_rows), "detalle": "Facturas individuales con criterio de riesgo"},
        {"indicador": "Proveedores con riesgo RR9", "valor": len(rr9_rows), "detalle": "RFC con concentracion, cancelaciones o patrones"},
        {
