...
from app.modules.bank_shield.services.scoring.rules import (
    RFC_MATCH_SCORE_POINTS,
    SUPPLIER_NAME_MATCH_SCORE_POINTS,
    SUPPLIER_PARTIAL_MATCH_SCORE_POINTS,
)
...
def _supplier_match_score(transaction: ParsedBankTransaction, invoice: Invoice) -> tuple[int, str | None]:
    ...
    for rfc_value in (invoice.rfc_emisor, getattr(invoice, "rfc_receptor", None)):
        ...
        if normalized_rfc and normalized_rfc in haystack:
            return RFC_MATCH_SCORE_POINTS, "RFC detectado en descripcion"
    ...
    if supplier_name and len(supplier_name) >= 6 and supplier_name in haystack:
        return SUPPLIER_NAME_MATCH_SCORE_POINTS, "Proveedor detectado en descripcion"
    ...
    if len(supplier_tokens) >= 2 and haystack_tokens:
        ...
        if len(matched_tokens) >= 2:
            ...
            if coverage >= 0.6:
                return SUPPLIER_PARTIAL_MATCH_SCORE_POINTS, "Coincidencia por proveedor/nombre"
